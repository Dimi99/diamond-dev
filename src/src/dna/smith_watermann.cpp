...
namespace SmithWaterman{
    ...
    int scoringFunction(int match, int mismatch, Letter first,Letter second){return first==second ? match : mismatch;}

    int dynamic_program(const Sequence &target, const Sequence &query, int match, int mismatch, int gapopen, int gapextend){
    ...
    std::pair<vector<Extension::Match>, Extension::Stats> local_alignment(const Search::Config &cfg,BlockId query_id){
        ...
        for(int i = 0; i < target_seqs.size(); i ++){
            ...
            int score = dynamic_program(target_sequence, query_sequence,cfg.score_builder->reward(), cfg.score_builder->penalty(), cfg.score_builder->gap_open(), cfg.score_builder->gap_extend()) ;
