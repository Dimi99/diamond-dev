...
void Index::filter_worker(std::atomic<unsigned> *seedp, int index) {
    ...
    while ((part = (*seedp)++) < Const::seedp) {
        auto it = merge_keys(seed_arr_->begin(part), (seed_arr_->begin(part) + seed_arr_->size(part)),
                             SeedArray::Entry::GetKey());
        while (it.good()) {
                this->rep_thread[index].emplace(it.key(), it.count());
                this->rep_thread[index].pop();
            ++it;
        }
    }
}
...
int Index::filter_repetitive(const SeedPartitionRange &range) {
    if(this->n_minimizer * config.repetitive_cutoff < 1)
        return INT_MAX;
    std::function<void(atomic<unsigned> *seedp, int heap_index)> functor = [this](atomic<unsigned int> *PH1, int heap_index) { filter_worker(PH1,heap_index); };
    std::priority_queue<RepetitiveCounter,std::vector<RepetitiveCounter>,std::greater<>> repetitive_max;

    atomic<unsigned> seedp(range.begin());
    vector<std::thread> threads;
    std::vector<RepetitiveCounter> temp(this->n_minimizer * config.repetitive_cutoff);
    repetitive_max = std::priority_queue(temp.begin(), temp.end(), std::greater<>());

    this->rep_thread.reserve(config.threads_);

    for (int i = 0; i < config.threads_; ++i) {
        this->rep_thread.emplace_back(temp.begin(), temp.end(), std::greater<>());
        threads.emplace_back(functor, &seedp, i);
    }
    for (auto &t: threads)
        t.join();

    for(auto &heap: this->rep_thread){
        while (!heap.empty()) {
    ...
    return repetitive_max.top().count;
}
