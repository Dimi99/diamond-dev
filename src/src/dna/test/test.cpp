...
namespace DnaTest {
    ...
    int run() {
        ...
        FastaFile dna("test1", true, FastaFile::WriteAccess(), nucleotide_traits);

        shared_ptr<SequenceFile> query_file(new FastaFile("test2", true, FastaFile::WriteAccess(),nucleotide_traits)), db(new FastaFile("test3", true, FastaFile::WriteAccess(),nucleotide_traits));
